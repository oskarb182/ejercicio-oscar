 ...
 *
 */
public class Director implements IDispatcher {
	...
	public void dispatchCall(Llamada call) {
		...
		if (validarOperadores(call)) {
			System.out.println("LLamada atentida por un director");
			
		}else{
			System.out.println("Llamada " + call.getIdLlamada() + " no pudo ser atendida por un director " );
		 ...
		 boolean asignado = false;
			...
			for (Long key : empleados.keySet()) {
				if (empleados.get(key) == null) {
					System.out.println("Asignado llamada " + llamada.getIdLlamada() + " Duración " +llamada.getDuracion() +" al Director: " + key);
